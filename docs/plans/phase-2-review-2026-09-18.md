...
|---|---|---|---|
| 1 | Agents already teach in plain speech (the review said ~45% of utterances state a complete true recipe; zero TEACH). | Confirmed with corrected numbers: 30% name a pair the speaker had made (45% under the loose any-two-names reading), 15% state pair and product (20% loose); zero TEACH. | Plan §0, §2.2; Stage 0 `exposure.py` R2–R3 |
...
| 7 | Fix 1–6 with a strict deterministic matcher, speaker-holds rule, window, TEACH as upgrade. | Adopted as D1 (connector-joined pairs; speaker holds; utterance before claim; student naive; window 3). | Plan D1 |
...
| 15 | $0 schedule simulation must set every canary bar. | `lifespan.py` + `canary_check` (deaths by day, spread, heir days, overlap, generation by day); every schedule bar in Stage E is computed and pinned; behavioural bars are not simulator outputs. | Stage 0; Plan Stage E |
| 16 | v1's "ceiling ≈19 days" was an arithmetic error. | Corrected to 8.3–10.5 days at 200k. | Plan §2.3 |
| 17 | Most actions cost two calls (truncation); a large share of thinking qi goes to truncated first calls. | Measured by the retread tool: 94% of actions retried (1,801 after a truncated first reply), 47% of thinking qi on truncated first calls, reflections truncated 97%; the review's 85%/40% were a conflation, noted in plan §2.3 and the devlog errata; drives D10. | Plan §2.3; Stage 0 `retread.py` truncation |
...
| 44 | Transcript-generated heir breaks symmetry and replay. | Rejected: D4 requires no model call and no RNG. | Plan D4 |
| 45 | RNG-drawn heir diverges personas between arms. | Rejected; heir is a pure function of lineage. | Plan D4 |
| 46 | Inheritance vector unspecified; notes inheritance in oral mode. | Specified in D4 (stones reset, location inherited, everything else empty, no notes); pinned in `SuccessionConfig` at Stage A. | Plan D4 |
...
| 54 | Reorder: retro-provenance, minimal Stage A, teaching smoke, then the store. | Adopted in part: Stage 0 first and the smoke doubles as the mortality rung, but the provenance sidecar (B3) is built in Stage B before the smoke so E1 can run `blame`. | Plan §4 |
| 55 | Run one 30-day archive arm as the gate candidate before the other three. | F1 before F2. | Plan Stage F |
| 56 | E2 costs $68–82, not $40–55; Stage E heading said $6. | Budget rebuilt from $0.68 per arm-day, +15% for p3.0 (E1 ≈$13, E2 ≈$21, F1 ≈$23, F2 ≈$70, reserve ≈$21). | Plan §6 |
| 57 | Re-canary reserve unbudgeted. | Reserve line (≈$21). | Plan §6 |
| 58 | Realistic exposure $110–140; prompt caching cannot help. | Stated ($127 expected, $148 ceiling); caching noted as inapplicable. | Plan §6 |
| 59 | Canary "crosses a death" bar names no tier or novelty condition. | Bar: `taught` with student generation above the teacher's, tier ≥ 2, student naive (the born-after-death disjunct was dropped: an exposure requires delivery to a living listener). | Plan Stage E2 |
| 60 | Retread not a metric; baseline is a distinct count; bar contradicts the risk table. | Pair-level own-journal retread per life-day (metric baseline: the accepted run) and `ancestral` retread defined; the canary bar compares heirs to same-arm founders; only own-journal retread is barred. | Plan D9, Stage E2, §5; Stage 0 `retread.py` |
